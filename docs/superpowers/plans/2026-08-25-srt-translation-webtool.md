...
- Drust insert endpoint is `POST /t/{tenant}/records/{collection}` — **the request body must be wrapped as `{"data": {...fields...}}`, not a flat field dict, and the response wraps the created row under `{"id": N, "record": {...}}`** (corrected 2026-08-25 during Task 4 implementation: the tenant's published OpenAPI doc claims a flat body/response, but a live probe against the real tenant returned `422 Unprocessable Entity — missing field 'data'` for a flat body, and a flat-body assumption was independently re-confirmed wrong via `GET /t/{tenant}/records/pending_terms/{id}`, which also wraps under `record`). `webtool/drust_client.py`'s `insert_pending_term` implements the corrected contract; its Python-level signature/return value are unchanged, so no other task's code needed to change.
...
- Anon token: `<REDACTED-rotated, see .env>`. Service token: `<REDACTED-rotated, see .env>`.
...
- Produces (used by every later task): `webtool/config.py` module-level constants:
  ...
  - `DRUST_ANON_TOKEN = "<REDACTED-rotated, see .env>"`
  - `DRUST_SERVICE_TOKEN = "<REDACTED-rotated, see .env>"`
